...
function CardSauce({ sauce, cart }) {
  ...
  const addToCart = () => {
    if (admin) {
      const test = doc(db, `cart-${admin.uid}`, `article-${sauce.id}`);

      setDoc(test, sauce, { merge: true }).then(() => {
        setSuccess(true);
      });
    ...
    <div className="flex flex-col items-center justify-end relative w-1/4 h-[250px] bg-[#fff] shadow-xl  rounded-xl mr-4 mb-10">
      ...
      <>
        <div className="text-center my-5 px-4">
          <h4 className="text-lg font-medium mb-2">{sauce.name}</h4>
          <p className="text-sm text-gray-400">{sauce.description}</p>
        </div>
        ...
        <div className="flex justify-between w-[85%]">
          <span className="font-bold">{sauce.prix}€</span>
