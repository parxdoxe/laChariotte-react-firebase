...
function CardPlats({plat}) {
  ...
  const addToCart = () => {
    if (admin) {
      const test = doc(db, `cart-${admin.uid}`, `article-${plat.id}`);

      setDoc(test, plat, { merge: true }).then(() => {
        setSuccess(true);
      });
  ...
  const changHeart = () => {
    ...
    return (
        <div className="flex flex-col items-center justify-end relative w-1/4 h-[250px] bg-[#fff] shadow-xl  rounded-xl mr-4 mb-10">
      <img
      ...
      <>
        <div className="text-center my-5 px-4">
          <h4 className="text-lg font-medium mb-2">{plat.name}</h4>
          <p className="text-sm text-gray-400">{plat.description}</p>
        </div>
        ...
        <div className="flex justify-between w-[85%]">
          <span className="font-bold">{plat.prix}€</span>
